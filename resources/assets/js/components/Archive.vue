<template>
	<div>
			...
			<div class="row">
				<div class="col-md-12">
					<div class="scroll-wrapper">
						<template v-if="board_lists.lenght != 0">
						...
						<template v-else>
							<div class="outer">
								<div class="middle">
									<div class="inner">
										<img :src="noData" alt="">
										<h3 style="color: #E74C3C;">No card found!</h3>
									</div>
								</div>
							</div>
						</template>
			...
			<div class="row">
				<div class="col-md-12" style="margin-top: 20px;">
					<div class="form-group">
						...
						<b-button variant="warning" size="sm" style="margin-top: 5px;" @click="sendToBoardList(selected_list.id)" block>Send to board</b-button>
						<b-button variant="danger" size="sm" style="margin-top: 5px;" @click="deleteArchivedBoardList(selected_list.id)" block>Delete Board</b-button>
					</div>
				</div>
			</div>
		</b-modal>
		...
		<b-modal id="visitcardmodal" centered ref="visitcardmodal" size="xl" hide-header hide-footer>
			<div class="row" style="padding: 10px; background-color: #e1dfdf; margin-top: 1px;">
				<p style="margin-bottom: 0px; font-family: AileronRegular;">This card is archived</p>
			</div>
			<div class="row p-4 mt-4">
				...
				<div class="col-md-3" style="margin-top: 20px;">
					...
					<div>
						...
						<div v-if="selected_card.labels_id != 0" :style="{ 'background-color': selected_card.color_name }" class="visitedLabelStyle">
							{{ selected_card.name }}
						</div>
						<div v-if="selected_card.labels_id == 0" style="background-color: #aeafb0; text-align: center;" class="visitedLabelStyle">
							No Label
						</div>
					</div>
					<hr>
					...
					<div>
						<strong>
							...
							<b-button variant="secondary" block @click="sendToBoard()">
								Send to Board
							</b-button>
						</strong>
					</div>
...
<script>
	export default {
		props: ['id', 'boardlists', 'cardlists', 'labellists', 'colorlists', 'checklists', 'checklistitems', 'cardattachment'],
		data() {
			return {
		...
		methods: {
			...
			getCards() {
				axios.get('../../board/'+ this.board_id +'/getArchiveCards')
				.then((response) => {
			...
			sendToBoard() {
				axios.post('../../board/'+ this.board_id +'/sendToBoard', {
					cardId: this.selected_card.id,
					boardListId: this.selected_card.board_lists_id
				})
				.then((response) => {
					this.getCards();
			...
			removeAttachment() {
				axios.post('../../board/' + this.board_id + '/archive/removeAttachment', {
					cardId: this.selected_card.id
				})
				.then((response) => {
					// this.$refs.visitcardmodal.hide();
			...
			sendToBoardList(id) {
				this.$swal({
					title: 'Are you sure to send this all to board?',
					...
					if (result.value) {
						axios.post('../../board/archive/sendToBoardList', {
							boardId: id
						})
						.then((response) => {
							this.$swal({
			...
			deleteArchivedBoardList(id) {
				this.$swal({
					...
					if (result.value) {
						axios.post('../../board/archive/deleteArchivedBoardList', {
							boardId: id
						})
						.then((response) => {
							this.$swal({
...
<style>
	body {
		background-color: #b63c30 !important;
	}
